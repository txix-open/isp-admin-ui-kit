...
# Script to update isp-admin-ui-kit version across multiple projects
...
echo "Updating isp-admin-ui-kit to version $NEW_VERSION"
...
for project in "${PROJECTS_ARRAY[@]}"; do
    if [ -d "$project" ]; then
        ...
        if grep -q "isp-admin-ui-kit" package.json 2>/dev/null; then
            # 1. Поднимаем версию в package.json (обновляем существующую запись)
            cat package.json | jq --arg ver "$NEW_VERSION" '
                if .dependencies and .dependencies["isp-admin-ui-kit"] then 
                    .dependencies["isp-admin-ui-kit"] = $ver 
                elif .devDependencies and .devDependencies["isp-admin-ui-kit"] then 
                    .devDependencies["isp-admin-ui-kit"] = $ver 
                end
...
- Обновлено
  - библиотека isp-admin-ui-kit ($NEW_VERSION)
...
"
            ...
            echo "Skipping $project - no isp-admin-ui-kit dependency"
        fi
