...
### Проверка на стенде без публикации в npm

В каталоге библиотеки выполните `npm run pack:stand`. Команда собирает
production-версию и создаёт `isp-admin-ui-kit-<version>.tgz` без публикации.

Скопируйте архив в `vendor/` проекта `isp-admin-ui`, затем в каталоге приложения
выполните (подставьте версию созданного архива):

```bash
npm install ./vendor/isp-admin-ui-kit-1.11.0.tgz
```

Закоммитьте в приложении архив, `package.json` и `package-lock.json` и запускайте
обычный деплой. Dockerfile должен копировать `vendor/` до установки зависимостей.
После каждой новой упаковки повторяйте установку архива, чтобы обновить checksum
в lock-файле. Для возврата на npm установите нужную опубликованную версию через
`npm install isp-admin-ui-kit@<version>`.
